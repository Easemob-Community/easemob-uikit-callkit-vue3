      ...
      <div class="demo-section">
        ...
        <div class="event-log-section">
          ...
          <div class="idb-stats" v-if="idbStats.total > 0">
            ...
            <span>当前 callId: {{ coreCallState.callId || '无' }}</span>
          </div>
          <div class="button-group">
...
watch(
  ...
  (newStatus) => {
    if (newStatus === CALL_STATUS.IN_CALL) {
      ...
      const target = coreCallState.calleeUserId || ''
...
const handleResetState = () => {
  // TODO: 通过 callkit-core 提供的 API 重置状态
  currentCallInfo.value = ''
  console.log('通话状态已重置')
  alert('通话状态已重置')
}
...
const getTargetCallId = async (): Promise<string | undefined> => {
  const currentCallId = coreCallState.callId || Logger.getInstance().getSessionId()
