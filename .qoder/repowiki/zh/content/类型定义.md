...
## 更新摘要
**变更内容**
- 新增 CallParams 和 GroupCallParams 类型定义，完善类型导出体系
- 增强 Provider 配置接口，支持批量用户信息获取
- 新增 UserInfoProvider 和 GroupInfoProvider 类型定义
- 扩展 ProviderConfig 接口，添加 getUserInfo 和 getGroupInfo 方法
- 更新 UserProfileService，实现批量用户信息解析功能
- 增强 CallService 对批量用户信息的支持
...
**更新** 本次更新重点新增了 CallParams 和 GroupCallParams 类型定义，完善了类型导出体系，为单人通话和群组通话提供了更强的类型安全保障。

## 项目结构
类型定义主要分布在以下模块：
- lib/types：库级对外 API 类型（如 EasemobChatCallKitOptions、CallKitInstance、UseCallKitReturn、CallParams、GroupCallParams 等）
...
- lib/composables/useCallKit.ts：组合式 API，使用 CallParams 和 GroupCallParams 类型
...
graph TB
subgraph "库级对外API"
A["lib/types.ts<br/>对外实例与组合式API类型<br/>新增：CallParams、GroupCallParams"]
...
subgraph "实现与Hook"
G["lib/composables/useCallKit.ts<br/>组合式API使用CallParams/GroupCallParams"]
H["callkit/hooks/useCameraDevices.ts<br/>设备类型"]
I["lib/modules/groupCall/signaling/GroupCallSignalingAdapter.ts<br/>群组通话信令适配器"]
...
- [lib/composables/useCallKit.ts:3-12](file://lib/composables/useCallKit.ts#L3-L12)
...
- **新增** CallParams 与 GroupCallParams 类型
  - CallParams：单人通话参数类型，包含 targetId、type、msg、userInfo 等字段，用于 useCallKit.call() 方法。
  - GroupCallParams：群组通话参数类型，包含 groupId、members、type、msg、groupName、groupAvatar、userInfo 等字段，用于 useCallKit.groupCall() 方法。
  - 典型用法：在调用组合式 API 时提供类型安全的参数验证和智能提示。
...
App->>CallKit : "通过useCallKit()获取API"
CallKit->>Service : "调用call()/groupCall()方法"
...
CallParams --> UseCallKitReturn : "单人通话参数"
GroupCallParams --> UseCallKitReturn : "群组通话参数"
```
...
章节来源
- [lib/types.ts:1-101](file://lib/types.ts#L1-L101)

### **新增** CallParams 与 GroupCallParams 类型详解
- CallParams 类型定义
  - targetId：目标用户 ID，必填字段
  - type：通话类型，字面量类型 "audio" | "video"
  - msg：邀请消息，可选字符串
  - userInfo：用户信息对象，包含 nickname 和 avatarURL 两个可选字段
- GroupCallParams 类型定义
  - groupId：群组 ID，必填字段
  - members：成员列表，字符串数组
  - type：通话类型，字面量类型 "audio" | "video"
  - msg：邀请消息，可选字符串
  - groupName：群组名称，可选字符串
  - groupAvatar：群组头像 URL，可选字符串
  - userInfo：用户信息对象，包含 nickname 和 avatarURL 两个可选字段
- 使用场景
  - 在 useCallKit.call() 方法中作为参数类型
  - 在 useCallKit.groupCall() 方法中作为参数类型
  - 提供编译时类型检查和 IDE 智能提示
...
**图表来源**
- [lib/types.ts:65-80](file://lib/types.ts#L65-L80)
- [lib/types.ts:85-100](file://lib/types.ts#L85-L100)

章节来源
- [lib/types.ts:65-100](file://lib/types.ts#L65-L100)
...
- 类型耦合关系
  ...
  - **新增** UseCallKitReturn 依赖 CallParams 和 GroupCallParams 类型。
- 外部依赖
...
CallParams["CallParams"] --> UseCallKit["UseCallKitReturn"]
GroupCallParams["GroupCallParams"] --> UseCallKit
UseCallKit --> State
UseCallKit --> Signal
```
...
- **新增** CallParams 和 GroupCallParams 类型问题
  - 问题：编译时报错，提示参数类型不匹配。
  - 原因：传入的参数缺少必需字段或类型不正确。
  - 修复：确保按照 CallParams/GroupCallParams 接口定义提供完整且正确的参数。
  - 建议：使用 TypeScript 的类型检查功能，确保参数符合接口定义。
...
- 建议排查步骤
  ...
  - **新增** 检查 CallParams 和 GroupCallParams 的参数完整性，确保所有必需字段都已提供。
...
**更新** 新增的CallParams和GroupCallParams类型定义，完善了类型导出体系，为单人通话和群组通话提供了更强的类型安全保障。同时，新增的Provider配置接口和批量用户信息获取功能，显著提升了大规模通话场景下的性能表现和用户体验。通过缓存机制和批量请求优化，减少了网络请求次数和UI渲染开销。
...
- 类型使用示例与最佳实践
  ...
  - **新增** CallParams使用：在调用 useCallKit.call() 时提供类型安全的参数，包括 targetId、type、msg、userInfo 等字段。
  - **新增** GroupCallParams使用：在调用 useCallKit.groupCall() 时提供类型安全的群组通话参数，包括 groupId、members、type、msg、groupName、groupAvatar、userInfo 等字段。
- 类型扩展与自定义
  ...
  - **新增** 参数类型扩展：如需扩展通话参数，可在 CallParams 或 GroupCallParams 基础上添加新的可选字段，保持向后兼容性。