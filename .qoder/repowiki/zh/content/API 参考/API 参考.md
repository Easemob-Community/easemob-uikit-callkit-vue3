...
- [lib/modules/groupCall/GroupCallShell.vue](file://lib/modules/groupCall/GroupCallShell.vue)
...
## 更新摘要
**所做更改**
- 基于 USAGE.md 重构为完整的 API 参考文档
- 新增完整的组件 API、组合式函数 API、Store API、服务 API 和类型定义
- 更新了所有组件的详细属性、事件和方法说明
- 新增拖拽定位组合式函数的完整 API 规范
- 更新了 Store API 的详细状态、getter 和 action 规范
- 新增常量和类型定义的完整参考

## 目录
1. [导出清单](#导出清单)
2. [组件 API](#组件-api)
3. [组合式函数 API](#组合式函数-api)
...
```typescript
const rtcService = new RtcService(config)
```
...
| `encoderConfig` | `VideoEncoderConfigurationPreset` | 视频编码配置 |
...
| `onVolumeIndicator` | `(volumes: any[]) => void` | 音量指示回调 |
| `chatClient` | `any` | 环信客户端实例，用于获取 userId 映射 |

#### 方法

| 方法 | 参数 | 返回值 | 说明 |
|------|------|--------|------|
| `initialize()` | — | `Promise<void>` | 初始化 RTC 客户端 |
| `setAppId(appId)` | `string` | `void` | 设置 App ID |
| `joinChannel(channelName, token, uid, appId?)` | `string, string \| null, number \| string, string?` | `Promise<number \| string>` | 加入频道 |
| `leaveChannel()` | — | `Promise<void>` | 离开频道 |
| `createAudioTrack()` | — | `Promise<IMicrophoneAudioTrack>` | 创建音频轨道 |
| `createVideoTrack()` | — | `Promise<ICameraVideoTrack>` | 创建视频轨道 |
| `publishTracks(tracks)` | `IAgoraRTCRemoteUser[]` | `Promise<void>` | 发布轨道 |
| `toggleAudio(enabled)` | `boolean` | `Promise<boolean>` | 切换音频 |
| `toggleVideo(enabled)` | `boolean` | `Promise<boolean>` | 切换视频 |
| `switchCamera(deviceId)` | `string?` | `Promise<boolean>` | 切换摄像头 |
| `switchMicrophone(deviceId)` | `string?` | `Promise<boolean>` | 切换麦克风 |
| `subscribeRemoteUser(user, mediaType)` | `IAgoraRTCRemoteUser, 'audio' \| 'video'` | `Promise<void>` | 订阅远程用户 |
| `getLocalVideoStream()` | — | `MediaStream \| null` | 获取本地视频流 |
| `getRemoteVideoTrack(userId)` | `string` | `IRemoteVideoTrack \| null` | 获取远程视频轨道 |
| `getRemoteAudioTrack(userId)` | `string` | `IRemoteAudioTrack \| null` | 获取远程音频轨道 |
| `isMuted()` | — | `boolean` | 检查是否静音 |
| `isCameraEnabled()` | — | `boolean` | 检查摄像头是否启用 |
| `getClient()` | — | `IAgoraRTCClient \| null` | 获取客户端实例 |
| `destroy()` | — | `Promise<void>` | 销毁服务 |
