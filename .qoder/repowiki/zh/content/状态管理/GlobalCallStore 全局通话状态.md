...
**问题描述**: batchSetUserInfo 方法无法正确设置用户信息

**解决方案**:
1. 验证 entries 参数格式是否正确
2. 检查每个 entry 的 userId 和 userInfo 字段
3. 确认 userInfo 对象包含正确的字段（nickname、avatarURL）
4. 验证用户ID的唯一性和有效性
5. 检查批量设置操作是否在合适的时机执行
