# 技术架构

<cite>
**本文档引用的文件**
- [README.md](file://README.md)
- [package.json](file://package.json)
- [lib/index.ts](file://lib/index.ts)
- [lib/components/EasemobChatCallKitProvider.vue](file://lib/components/EasemobChatCallKitProvider.vue)
- [lib/store/callState.ts](file://lib/store/callState.ts)
- [lib/store/rtcChannel.ts](file://lib/store/rtcChannel.ts)
- [lib/composables/useCallKit.ts](file://lib/composables/useCallKit.ts)
- [lib/composables/useListenerManager.ts](file://lib/composables/useListenerManager.ts)
- [lib/services/RtcService.ts](file://lib/services/RtcService.ts)
- [lib/types/callstate.types.ts](file://lib/types/callstate.types.ts)
- [lib/core/sdk/imSDK/index.ts](file://lib/core/sdk/imSDK/index.ts)
- [vite.lib.config.ts](file://vite.lib.config.ts)
- [QUICK_START.md](file://QUICK_START.md)
- [USAGE.md](file://USAGE.md)
</cite>

## 更新摘要
**变更内容**
- 更新依赖管理策略：SDKs现在作为peerDependencies外部化，简化用户集成
- 新增自动Pinia注入机制：插件自动检测并注入Pinia，无需用户手动配置
- 更新架构图以反映新的依赖管理模式
- 增强集成指南以说明新的依赖要求

## 目录
1. [引言](#引言)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖分析](#依赖分析)
7. [性能考虑](#性能考虑)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)
10. [附录](#附录)

## 引言
本项目为 Easemob Chat CallKit Vue3 插件，提供环信聊天与音视频通话能力的集成方案。本文档聚焦于整体技术架构，阐述分层架构模式（Provider-Store-Service-Component）、MVVM 设计思想在 Vue3 中的应用，以及组合式函数（Composables）的采用方式。文档同时解释技术选型原因（如 Vue3 Composition API、Pinia 状态管理、TypeScript、Agora RTC SDK、环信 Web SDK），并通过架构图与组件关系图帮助读者快速理解系统设计。

**更新** 本版本引入了新的依赖管理策略，SDKs现在作为peerDependencies外部化，以及自动Pinia注入的架构改进，显著简化了用户集成流程。

## 项目结构
项目采用"库模式"构建，核心源码位于 lib/ 目录，提供可复用的 Vue3 插件能力；同时保留 callkit/ 目录用于 React 版本的 CallKit（与 Vue3 插件并行）。测试样例位于 test/ 目录，支持源码模式与 tgz 包模式两种验证方式。

```mermaid
graph TB
subgraph "库模式(lib)"
LIDX["lib/index.ts<br/>插件入口与导出"]
PROVIDER["EasemobChatCallKitProvider.vue<br/>全局Provider"]
STORE["store/*<br/>Pinia Store"]
COMPOSABLES["composables/*<br/>组合式函数"]
SERVICES["services/*<br/>业务服务"]
TYPES["types/*<br/>类型定义"]
CORE["core/sdk/imSDK<br/>环信SDK封装"]
end
subgraph "构建与发布"
VITE["vite.lib.config.ts<br/>库构建配置"]
PKG["package.json<br/>依赖与导出配置"]
end
LIDX --> PROVIDER
LIDX --> STORE
LIDX --> COMPOSABLES
LIDX --> SERVICES
LIDX --> TYPES
LIDX --> CORE
VITE --> LIDX
PKG --> VITE
```

**图表来源**
- [lib/index.ts:1-99](file://lib/index.ts#L1-L99)
- [vite.lib.config.ts:1-69](file://vite.lib.config.ts#L1-L69)
- [package.json:1-55](file://package.json#L1-L55)

**章节来源**
- [README.md:5-31](file://README.md#L5-L31)
- [package.json:1-55](file://package.json#L1-L55)
- [vite.lib.config.ts:1-69](file://vite.lib.config.ts#L1-L69)

## 核心组件
- **Provider 层**：EasemobChatCallKitProvider.vue 负责全局配置注入、事件监听器挂载、RTC 服务初始化与销毁，确保上层组件在运行时具备一致的上下文。**新增**：自动检测并注入Pinia实例，无需用户手动配置。
- **Store 层**：基于 Pinia 的 callState 与 rtcChannel 两个 Store，分别管理通话状态机与 RTC 频道/媒体流状态，提供响应式状态与计算属性。
- **Service 层**：RtcService 封装 Agora WebRTC 能力，负责客户端初始化、加入/离开频道、音视频轨道管理、设备切换、网络质量与音量回调等。
- **Composables 层**：useCallKit、useListenerManager 等组合式函数封装业务流程与跨组件共享逻辑，降低组件复杂度。
- **Types 层**：统一的通话状态、类型常量与事件枚举，保证各模块间契约清晰。
- **Core 层**：对环信 Web SDK 的轻量封装，提供稳定的 IM 能力接入点。

**章节来源**
- [lib/components/EasemobChatCallKitProvider.vue:1-155](file://lib/components/EasemobChatCallKitProvider.vue#L1-L155)
- [lib/store/callState.ts:1-215](file://lib/store/callState.ts#L1-L215)
- [lib/store/rtcChannel.ts:1-264](file://lib/store/rtcChannel.ts#L1-L264)
- [lib/services/RtcService.ts:1-719](file://lib/services/RtcService.ts#L1-L719)
- [lib/composables/useCallKit.ts:1-246](file://lib/composables/useCallKit.ts#L1-L246)
- [lib/composables/useListenerManager.ts:1-262](file://lib/composables/useListenerManager.ts#L1-L262)
- [lib/types/callstate.types.ts:1-93](file://lib/types/callstate.types.ts#L1-L93)
- [lib/core/sdk/imSDK/index.ts:1-12](file://lib/core/sdk/imSDK/index.ts#L1-L12)

## 架构总览
整体采用"Provider-Store-Service-Component"的分层架构，结合 MVVM 思想（Model-View-ViewModel）在 Vue3 中落地：Store 承担 ViewModel 的职责，组件作为 View，Service 与 Composables 作为 Model 的一部分，共同协作完成端到端的通话体验。

**更新** 新的依赖管理模式下，SDKs作为peerDependencies外部化，插件通过自动Pinia注入简化集成流程。

```mermaid
graph TB
subgraph "视图层(View)"
UI["组件树<br/>EasemobChatSingleCall / EasemobChatMultiCall / InvitationNotification / EasemobChatMiniWindow"]
end
subgraph "视图模型层(ViewModel)"
PINIA["Pinia Store<br/>useCallStateStore / useRtcChannelStore"]
end
subgraph "模型层(Model)"
LISTENER["useListenerManager<br/>信令/文本消息监听"]
CALLKIT["useCallKit<br/>发起/加入通话"]
RTC["RtcService<br/>WebRTC封装"]
IM["imSDK封装<br/>环信SDK"]
end
subgraph "提供者层(Provider)"
PROVIDER["EasemobChatCallKitProvider<br/>全局配置/事件挂载/RTC初始化"]
end
subgraph "外部依赖管理"
PEER["peerDependencies<br/>vue, agora-rtc-sdk-ng, easemob-websdk"]
AUTO["自动Pinia注入<br/>createPinia()"]
end
UI --> PINIA
UI --> CALLKIT
UI --> LISTENER
PINIA --> RTC
PINIA --> IM
CALLKIT --> LISTENER
LISTENER --> RTC
PROVIDER --> LISTENER
PROVIDER --> RTC
PROVIDER --> IM
PEER --> PROVIDER
AUTO --> PROVIDER
```

**图表来源**
- [lib/components/EasemobChatCallKitProvider.vue:1-155](file://lib/components/EasemobChatCallKitProvider.vue#L1-L155)
- [lib/store/callState.ts:1-215](file://lib/store/callState.ts#L1-L215)
- [lib/store/rtcChannel.ts:1-264](file://lib/store/rtcChannel.ts#L1-L264)
- [lib/composables/useListenerManager.ts:1-262](file://lib/composables/useListenerManager.ts#L1-L262)
- [lib/composables/useCallKit.ts:1-246](file://lib/composables/useCallKit.ts#L1-L246)
- [lib/services/RtcService.ts:1-719](file://lib/services/RtcService.ts#L1-L719)
- [lib/core/sdk/imSDK/index.ts:1-12](file://lib/core/sdk/imSDK/index.ts#L1-L12)
- [package.json:33-40](file://package.json#L33-L40)

## 详细组件分析

### Provider 组件：全局配置与生命周期管理
- **职责**
  - 合并默认配置与用户配置，形成全局有效配置。
  - 初始化并注入环信客户端实例到 Store。
  - **新增**：自动检测并注入Pinia实例，无需用户手动安装。
  - 在组件挂载时创建并初始化 RTC 服务（占位 appId，实际 appId 由信令动态下发）。
  - 挂载文本消息与信令监听器，统一处理通话邀请与状态变更。
  - 组件卸载时销毁 RTC 服务，释放资源。
- **交互关系**
  - 依赖 useCallStateStore、useRtcChannelStore、useListenerManager。
  - 与 RtcService、imSDK 封装协同工作。

```mermaid
sequenceDiagram
participant App as "应用"
participant Provider as "EasemobChatCallKitProvider"
participant Store as "Pinia Store"
participant Listener as "useListenerManager"
participant RTC as "RtcService"
App->>Provider : 传入 chatClient/agoraAppId/initConfig
Provider->>Store : setClient(chatClient)
Provider->>Store : 初始化 inviteTimeout / 日志级别
Provider->>Store : 自动检测并注入Pinia
Provider->>RTC : initializeRtcService(占位appId)
Provider->>Listener : mountTextMessageListener()
Provider->>Listener : mountSignalListener()
Note over Provider,RTC : 组件卸载时 destroyRtcService()
```

**图表来源**
- [lib/components/EasemobChatCallKitProvider.vue:1-155](file://lib/components/EasemobChatCallKitProvider.vue#L1-L155)
- [lib/composables/useListenerManager.ts:201-262](file://lib/composables/useListenerManager.ts#L201-L262)
- [lib/services/RtcService.ts:78-104](file://lib/services/RtcService.ts#L78-L104)

**章节来源**
- [lib/components/EasemobChatCallKitProvider.vue:1-155](file://lib/components/EasemobChatCallKitProvider.vue#L1-L155)

### Store 层：状态管理与计算属性
- **callState Store**
  - 管理通话状态机（IDLE/INVITING/ALERTING/CONFIRM_RING/RECEIVED_CONFIRM_RING/ANSWER_CALL/CONFIRM_CALLEE/IN_CALL）。
  - 维护邀请信息、用户映射、定时器、最小化窗口状态等。
  - 提供超时计时器、状态重置、邀请成员管理等动作与 getter。
- **rtcChannel Store**
  - 管理 RTC 频道、参与者集合、UID/UserId 映射、本地/远端媒体流、通话计时器等。
  - 提供初始化/销毁 RTC 服务、加入/离开频道、设置媒体状态、映射管理等动作与 getter。
- **MVVM 视图模型映射**
  - Store 的 state 作为 Model，getter 作为 ViewModel 的计算属性，组件通过响应式绑定展示状态。

```mermaid
classDiagram
class CallStateStore {
+state : CallState
+actions : initCallState(), initInviteInfo(), startTimeoutTimer(), setCallStatus(), resetCallState()
+getters : getCallStatus(), getCallState(), getUserInfo(), isInviting(), isInCall(), getInvitedMembers(), getIsMinimized()
}
class RtcChannelStore {
+state : RtcChannelState
+actions : initializeRtcService(), destroyRtcService(), createChannel(), joinChannel(), leaveChannel(), setLocalStream(), setAudioEnabled(), setVideoEnabled(), startCallTimer(), stopCallTimer(), setUidToUserIdMapping(), markUserJoinedRtc(), markUserLeftRtc(), clearLeftUsers(), reset()
+getters : activeChannel(), activeChannelParticipantCount(), channelIds(), getRtcService(), formattedCallDuration()
}
CallStateStore --> RtcChannelStore : "协作(状态联动)"
```

**图表来源**
- [lib/store/callState.ts:1-215](file://lib/store/callState.ts#L1-L215)
- [lib/store/rtcChannel.ts:1-264](file://lib/store/rtcChannel.ts#L1-L264)

**章节来源**
- [lib/store/callState.ts:1-215](file://lib/store/callState.ts#L1-L215)
- [lib/store/rtcChannel.ts:1-264](file://lib/store/rtcChannel.ts#L1-L264)

### Service 层：WebRTC 与 IM 能力封装
- **RtcService**
  - 负责 Agora WebRTC 客户端生命周期、加入/离开频道、本地/远端轨道管理、设备切换、网络质量与音量回调。
  - 与 rtcChannel Store 协作维护 UID/UserId 映射、用户加入/离开状态、本地流同步。
- **imSDK 封装**
  - 对 easemob-websdk 的类型化封装，提供稳定的 IM 能力接入点，供 Provider 与监听器使用。

```mermaid
classDiagram
class RtcService {
-client : IAgoraRTCClient
-appId : string
-localVideoTrack : ICameraVideoTrack
-localAudioTrack : IMicrophoneAudioTrack
-remoteVideoTracks : Map
-remoteAudioTracks : Map
+initialize() : Promise~void~
+joinChannel(channelName, token, uid, appId?) : Promise~number|string~
+leaveChannel() : Promise~void~
+createAudioTrack() : Promise~IMicrophoneAudioTrack~
+createVideoTrack() : Promise~ICameraVideoTrack~
+publishTracks(tracks) : Promise~void~
+toggleAudio(enabled) : Promise~boolean~
+toggleVideo(enabled) : Promise~boolean~
+switchCamera(deviceId) : Promise~boolean~
+switchMicrophone(deviceId) : Promise~boolean~
+subscribeRemoteUser(user, mediaType) : Promise~void~
+getLocalVideoStream() : MediaStream|null
+getRemoteVideoTrack(userId) : IRemoteVideoTrack|null
+getRemoteAudioTrack(userId) : IRemoteAudioTrack|null
+isMuted() : boolean
+isCameraEnabled() : boolean
+getClient() : IAgoraRTCClient|null
+destroy() : Promise~void~
}
class ImSDKWrapper {
+ChatSDK
+类型别名 : Chat, ChatSDKStatic
}
RtcService --> RtcChannelStore : "写入/读取状态"
RtcService --> ImSDKWrapper : "获取UID映射"
```

**图表来源**
- [lib/services/RtcService.ts:1-719](file://lib/services/RtcService.ts#L1-L719)
- [lib/store/rtcChannel.ts:1-264](file://lib/store/rtcChannel.ts#L1-L264)
- [lib/core/sdk/imSDK/index.ts:1-12](file://lib/core/sdk/imSDK/index.ts#L1-L12)

**章节来源**
- [lib/services/RtcService.ts:1-719](file://lib/services/RtcService.ts#L1-L719)
- [lib/core/sdk/imSDK/index.ts:1-12](file://lib/core/sdk/imSDK/index.ts#L1-L12)

### Composables 层：业务流程与共享逻辑
- **useCallKit**
  - 封装发起单人/群组通话的流程：初始化邀请信息、发送邀请消息、主叫方加入频道等。
- **useListenerManager**
  - 统一处理文本消息（邀请）与信令消息（alert/confirmRing/answerCall/confirmCallee/cancelCall/leaveCall），驱动 Store 状态变更与 RTC 行为。
- **与 MVVM 的关系**
  - Composables 作为 ViewModel 的一部分，承载业务逻辑，组件通过响应式 Store 与回调感知状态变化。

```mermaid
flowchart TD
Start(["useCallKit.startSingleCall"]) --> InitInvite["初始化邀请信息<br/>setInviteInfo()"]
InitInvite --> SendInvite["发送邀请消息<br/>sendInviteMessage()"]
SendInvite --> End(["完成"])
Start2(["useCallKit.startGroupCall"]) --> InitGroupInvite["初始化群组邀请信息"]
InitGroupInvite --> SendGroupInvite["发送群组邀请消息"]
SendGroupInvite --> JoinChannel["主叫方加入频道<br/>joinChannel()"]
JoinChannel --> End2(["完成"])
```

**图表来源**
- [lib/composables/useCallKit.ts:1-246](file://lib/composables/useCallKit.ts#L1-L246)

**章节来源**
- [lib/composables/useCallKit.ts:1-246](file://lib/composables/useCallKit.ts#L1-L246)
- [lib/composables/useListenerManager.ts:1-262](file://lib/composables/useListenerManager.ts#L1-L262)

### 类型体系：统一契约与可维护性
- **通话状态与类型常量**：CALL_STATUS、CALL_TYPE、HANGUP_REASON 等，确保跨模块一致性。
- **类型导出**：在 lib/index.ts 中集中导出，便于上层按需引入。

**章节来源**
- [lib/types/callstate.types.ts:1-93](file://lib/types/callstate.types.ts#L1-L93)
- [lib/index.ts:42-81](file://lib/index.ts#L42-L81)

### 构建与发布：库模式与类型声明
- **构建配置**
  - 使用 vite.lib.config.ts 输出 ES 与 UMD 两种格式，CSS 独立打包，Rollup 外部化 vue 与 pinia。
  - 构建前自动清空 release/dist 目录，确保产物纯净。
- **类型声明**
  - 通过 vite-plugin-dts 生成类型声明文件，输出到 release/dist。

**章节来源**
- [vite.lib.config.ts:1-69](file://vite.lib.config.ts#L1-L69)
- [package.json:1-55](file://package.json#L1-L55)

## 依赖分析
- **运行时依赖**
  - **更新**：Vue3：组件框架与 Composition API。
  - **更新**：Pinia：状态管理（现在由插件自动注入）。
  - **更新**：Agora RTC SDK NG：WebRTC 能力（作为peerDependencies外部化）。
  - **更新**：环信 Web SDK：即时通讯能力（作为peerDependencies外部化）。
- **开发依赖**
  - TypeScript、Vite、vue-tsc、vite-plugin-dts 等，保障类型安全与高质量构建。

**更新** 依赖管理模式的重大变化：
- SDKs现在作为peerDependencies外部化，用户需要自行安装环信和声网SDK
- Pinia现在作为普通依赖存在，但插件会自动检测并注入
- 这种设计允许用户控制SDK版本，避免版本冲突

```mermaid
graph LR
Vue["vue"] --> APP["应用"]
Pinia["pinia"] --> APP
Agora["agora-rtc-sdk-ng"] --> RTC["RtcService"]
Easemob["easemob-websdk"] --> IM["imSDK封装"]
APP --> Provider["Provider"]
Provider --> RTC
Provider --> IM
Provider --> Store["Pinia Store"]
Store --> RTC
Store --> IM
```

**图表来源**
- [package.json:33-40](file://package.json#L33-L40)

**章节来源**
- [package.json:1-55](file://package.json#L1-L55)

## 性能考虑
- **组合式函数与响应式 Store**：减少重复渲染与副作用，提升组件性能与可维护性。
- **RTC 轨道管理**：按需创建/销毁本地轨道，避免重复发布与资源泄漏。
- **事件监听与定时器**：在 Provider 卸载时统一销毁，防止内存泄漏。
- **构建优化**：库模式输出独立 CSS 与类型声明，减小应用包体积。
- **更新**：**自动Pinia注入**：避免重复创建Pinia实例，减少不必要的初始化开销。

## 故障排查指南
- **Provider 未挂载事件监听器**
  - 检查是否传入 chatClient；确认 Provider 组件已挂载。
- **RTC 初始化失败**
  - 确认 Agora appId 是否在加入频道时动态下发；检查 RtcService.initialize 与 joinChannel 的调用顺序。
- **信令处理异常**
  - 查看 useListenerManager 的日志输出，确认 onTextMessage 与 onCmdMessage 的挂载与过滤条件。
- **超时与状态不一致**
  - 检查 callState 的定时器与状态流转；多人通话场景下超时不自动隐藏界面属预期行为。
- **更新**：**Pinia注入问题**
  - 确认应用已正确安装Pinia；检查插件是否在应用启动时正确调用；查看控制台是否有Pinia相关的警告信息。

**章节来源**
- [lib/components/EasemobChatCallKitProvider.vue:93-155](file://lib/components/EasemobChatCallKitProvider.vue#L93-L155)
- [lib/composables/useListenerManager.ts:201-262](file://lib/composables/useListenerManager.ts#L201-L262)
- [lib/store/callState.ts:60-131](file://lib/store/callState.ts#L60-L131)

## 结论
本项目通过 Provider-Store-Service-Component 的分层架构，结合 Vue3 Composition API 与 Pinia，实现了高内聚、低耦合的音视频通话解决方案。**更新**：新的依赖管理策略（SDKs作为peerDependencies外部化）和自动Pinia注入机制显著简化了用户集成流程，提高了版本兼容性和灵活性。类型系统与构建配置进一步提升了可维护性与可复用性。建议在实际集成中遵循 Provider 全局初始化、Store 响应式驱动、Service 封装边界清晰的原则，确保系统稳定与性能。

## 附录
- **插件入口与导出**
  - lib/index.ts 汇集组件、Store、Composables、Service 与类型导出，作为插件统一入口。
  - **新增**：自动Pinia注入功能，通过 createPinia() 实现。
- **React 版本对照**
  - callkit/index.ts 与 callkit/CallKit.tsx 提供 React 版本实现，便于理解跨框架设计差异。
- **集成指南**
  - **前置条件**：Vue 3 项目、已安装并初始化 Pinia、已安装环信 IM SDK 和声网 RTC SDK。
  - **更新**：现在Pinia会自动注入，用户只需确保已安装即可。

**章节来源**
- [lib/index.ts:1-99](file://lib/index.ts#L1-L99)
- [QUICK_START.md:9-18](file://QUICK_START.md#L9-L18)
- [USAGE.md:56-90](file://USAGE.md#L56-L90)