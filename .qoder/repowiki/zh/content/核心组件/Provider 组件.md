...
- [lib/composables/useSignalManager.ts](file://lib/composables/useSignalManager.ts#L1-L200)
...
- [lib/composables/useListenerManager.ts](file://lib/composables/useListenerManager.ts#L620-L677)
...
章节来源
- [lib/components/EasemobChatCallKitProvider.vue](file://lib/components/EasemobChatCallKitProvider.vue#L65-L103)
...
- [lib/store/rtcChannel.ts](file://lib/store/rtcChannel.ts#L114-L121)
...
- Provider 在初始化阶段根据 initConfig.debug 设置日志级别
- 通过日志定位监听器挂载、RTC 初始化、信令处理等关键路径

章节来源
- [lib/utils/logger.ts](file://lib/utils/logger.ts#L91-L94)
- [lib/components/EasemobChatCallKitProvider.vue](file://lib/components/EasemobChatCallKitProvider.vue#L70-L76)
...
- 日志级别控制：生产环境建议关闭 VERBOSE，降低控制台输出开销

## 故障排查指南
- 未挂载事件监听器
  - 现象：Provider 输出“未挂载事件监听器：缺少环信客户端实例”
  - 排查：确认在 Provider 外层已传入 chatClient，且在 Provider 挂载后再设置
  - 参考：[lib/components/EasemobChatCallKitProvider.vue](file://lib/components/EasemobChatCallKitProvider.vue#L100-L102)

- RTC 初始化失败
  - 现象：日志报错“RTC服务初始化失败”
  - 排查：确认环信客户端已登录并可获取 userId 映射；检查占位 appId 的使用是否符合预期
  - 参考：[lib/store/rtcChannel.ts](file://lib/store/rtcChannel.ts#L105-L108)

- 邀请超时行为异常
  - 现象：多人通话超时后界面未自动隐藏
  - 说明：多人通话场景下超时后保持界面等待用户手动挂断，属设计行为
  - 参考：[lib/store/callState.ts](file://lib/store/callState.ts#L118-L127)

- 日志级别不符合预期
  - 现象：日志过多或过少
  - 排查：确认 initConfig.debug 设置；Provider 会在初始化阶段调用 logger.setDebug
  - 参考：[lib/components/EasemobChatCallKitProvider.vue](file://lib/components/EasemobChatCallKitProvider.vue#L70-L71)

- 组件卸载后仍有媒体占用
  - 现象：页面切换后摄像头/麦克风仍被占用
  - 排查：确认 Provider 已正确卸载；RtcService.destroy 会停止所有轨道并清理事件
  - 参考：[lib/services/RtcService.ts](file://lib/services/RtcService.ts#L678-L719)

章节来源
- [lib/components/EasemobChatCallKitProvider.vue](file://lib/components/EasemobChatCallKitProvider.vue#L100-L103)
- [lib/store/rtcChannel.ts](file://lib/store/rtcChannel.ts#L105-L108)
- [lib/store/callState.ts](file://lib/store/callState.ts#L118-L127)
- [lib/utils/logger.ts](file://lib/utils/logger.ts#L91-L94)
- [lib/services/RtcService.ts](file://lib/services/RtcService.ts#L678-L719)

## 结论
EasemobChatCallKitProvider 作为 CallKit 的根组件，承担了全局配置、状态初始化与事件监听的关键职责。通过与 Pinia Store、监听器管理器、RTC 服务的协作，实现了 IM 信令与 RTC 能力的无缝集成。遵循本文档的最佳实践与排障建议，可确保在复杂场景下稳定运行。
