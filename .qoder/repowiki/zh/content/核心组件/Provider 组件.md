...
## 更新摘要
**变更内容**
- ProviderConfig 新增 getUserInfo 和 getGroupInfo 配置选项，支持用户信息提供者配置
- 新增 UserProfileService 服务，提供批量用户信息获取和自动清理机制
- 增强用户资料缓存管理，支持全局用户信息映射和批量设置
- 更新 Provider 组件初始化流程，自动注册用户/群组信息提供者
- 新增组件卸载时的 Provider 清理机制，防止内存泄漏
...
**图表来源**
- [lib/components/EasemobChatCallKitProvider.vue:1-133](file://lib/components/EasemobChatCallKitProvider.vue#L1-L133)
...
- [lib/services/UserProfileService.ts:1-89](file://lib/services/UserProfileService.ts#L1-L89)
...
**更新** 新增用户信息提供者配置选项，支持批量用户信息获取和自动清理机制
...
Provider->>UserSvc : registerUserInfoProvider(getUserInfo)
Provider->>UserSvc : registerGroupInfoProvider(getGroupInfo)
...
**图表来源**
- [lib/components/EasemobChatCallKitProvider.vue:33-131](file://lib/components/EasemobChatCallKitProvider.vue#L33-L131)
...
**章节来源**
- [lib/components/EasemobChatCallKitProvider.vue:65-131](file://lib/components/EasemobChatCallKitProvider.vue#L65-L131)

### 用户信息提供者注册与使用流程
- Provider 注册：在 watchEffect 中检测 getUserInfo 和 getGroupInfo，分别调用 registerUserInfoProvider 和 registerGroupInfoProvider
...
**更新** 新增用户信息提供者配置和自动清理机制

**章节来源**
- [lib/components/EasemobChatCallKitProvider.vue:110-120](file://lib/components/EasemobChatCallKitProvider.vue#L110-L120)
- [lib/services/UserProfileService.ts:25-88](file://lib/services/UserProfileService.ts#L25-L88)
...
- 用户信息提供者未生效
  ...
  - 参考：[lib/components/EasemobChatCallKitProvider.vue:110-120](file://lib/components/EasemobChatCallKitProvider.vue#L110-L120)
...
- Provider 内存泄漏
  ...
  - 参考：[lib/components/EasemobChatCallKitProvider.vue:127-131](file://lib/components/EasemobChatCallKitProvider.vue#L127-L131)
...
## 结论
EasemobChatCallKitProvider 作为 CallKit 的根组件，承担了全局配置、状态初始化、事件监听和用户信息管理的关键职责。通过与 Pinia Store、监听器管理器、RTC 服务、用户信息提供者的协作，实现了 IM 信令与 RTC 能力的无缝集成，同时提供了完善的用户信息管理机制。新增的用户信息提供者配置选项支持批量用户信息获取和自动清理机制，显著提升了用户体验和系统性能。遵循本文档的最佳实践与排障建议，可确保在复杂场景下稳定运行。
...
**更新** 新增用户信息提供者配置选项，提供批量用户信息获取和自动清理机制
...
- 最佳实践
  ...
  - getUserInfo 和 getGroupInfo 应支持批量查询，提高性能
  - 确保在 Provider 卸载前完成所有通话资源清理
  - 用户信息提供者应处理查询失败的情况，返回兜底数据
