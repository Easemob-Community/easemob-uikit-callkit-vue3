...
- [modules/groupCall/index.ts](file://lib/modules/groupCall/index.ts)
...
- **新增** EasemobChatGroupMemberList 组件现已对外导出，增强了群组通话组件的可扩展性和开发者体验
...
**重要更新**：新版本完全重构了架构，移除了旧的 Vue 组件实现，采用基于 GroupCallShell 的现代化设计，提供了更好的可维护性和扩展性。**最新更新**：EasemobChatGroupMemberList 组件现已对外导出，为开发者提供了更大的灵活性和可扩展性。
...
- **新增** 对外导出的 EasemobChatGroupMemberList 组件
...
- [index.ts:1-18](file://lib/index.ts#L1-L18)
- [GroupCallShell.vue](file://lib/modules/groupCall/components/GroupCallShell.vue)
- [callState.ts](file://lib/store/callState.ts)
...
### EasemobChatGroupMemberList 成员列表组件

**新增** EasemobChatGroupMemberList 是对外导出的独立组件，专门用于群组成员邀请功能。
...
participant EasemobChatGroupMemberList as 成员列表组件
...
GroupCallShell->>EasemobChatGroupMemberList : 加载成员列表
EasemobChatGroupMemberList->>ChatClient : 获取群成员
ChatClient-->>EasemobChatGroupMemberList : 返回成员数据
EasemobChatGroupMemberList-->>GroupCallShell : 成员列表就绪
...
**新增** EasemobChatGroupMemberList 组件现已对外导出，显著增强了群组通话组件的可扩展性和开发者体验。
...
**章节来源**
- [index.ts:24-32](file://lib/index.ts#L24-L32)
...
GroupCallShell --> ParticipantTile
GroupCallShell --> VideoGrid
GroupCallShell --> EasemobChatGroupMemberList
GroupCallShell --> ViewModel
...
**成员列表加载问题**
- 检查 EasemobChatGroupMemberList 导出状态
- 验证群组 ID 和权限
- 确认网络连接和 SDK 配置
...
- **新增** 增强的参与者状态管理和 UI 组件
- **新增** 统一的图标管理和状态管理
- **新增** 对外导出的 EasemobChatGroupMemberList 组件，显著提升可扩展性和开发者体验
...
- 增强的错误处理和调试功能
- 更好的性能和可维护性
- **新增** 对外导出的成员列表组件，支持独立使用和组合扩展

**对外导出价值：**
- EasemobChatGroupMemberList 组件现已对外导出，开发者可以直接使用该组件
- 支持独立的成员邀请功能，无需完整的群组通话界面
- 提供了更大的灵活性，可以与其他组件自由组合
- 完整的 TypeScript 类型支持，确保开发体验

通过合理使用本组件的配置选项和自定义功能，开发者可以快速构建高质量的群组通话应用。新的模块化架构和状态管理功能进一步提升了组件的可定制性和用户体验。对外导出的 EasemobChatGroupMemberList 组件为开发者提供了更多可能性，使得群组通话功能的集成更加灵活和便捷。