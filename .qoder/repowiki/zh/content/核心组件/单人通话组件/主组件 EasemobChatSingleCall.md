...
- [lib/components/singleCall/EasemobChatSingleCall.vue:1-213](file://lib/components/singleCall/EasemobChatSingleCall.vue#L1-L213)
...
## 核心组件
- **EasemobChatSingleCall**：主组件，负责根据通话状态自动渲染"待接听"或"通话中"界面，控制大/小窗口切换，触发事件并暴露属性。**新增自动可见性管理**和**可选参数支持**，**新增 isGroupCall 计算属性**防止渲染冲突。
...
```mermaid
flowchart TD
Start(["组件挂载"]) --> AutoVisibility["自动可见性管理<br/>isCallVisible = status !== IDLE"]
AutoVisibility --> GroupCheck["群组通话检查<br/>isGroupCall = type === MULTI"]
GroupCheck --> UserInference["用户信息推断<br/>displayTargetUser = props.targetUser || store.user"]
