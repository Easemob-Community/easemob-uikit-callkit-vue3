...
- **新增功能**：增强的用户资料解析机制，自动解析目标用户的用户资料以确保呼叫等待屏幕显示前的信息可用
- 新增 useCallKitEvents 组合式 API，提供完整的事件订阅能力
- 改进错误处理和状态管理，增强 UI 卡住问题的兜底机制
- CallKitEventBus 提供类型安全的发布/订阅机制
- 完善群组通话的会话管理和参与者状态跟踪
- **useEndCall 增强**：使用 CALL_STATUS 枚举常量替代字符串比较，提供类型安全性，减少运行时错误风险
...
**重大更新**：本次更新反映了 useCallKit.ts 的重大增强，统一了单人和群组通话 API，新增了完整的事件发射机制，改进了错误处理和状态管理。**新增的关键变更**：useCallKit API 从位置参数改为结构化对象参数，新增 CallParams 和 GroupCallParams 类型支持，提供更好的类型安全和开发体验。**新增功能**：增强的用户资料解析机制，自动解析目标用户的用户资料以确保呼叫等待屏幕显示前的信息可用。**useEndCall 增强**：使用 CALL_STATUS 枚举常量替代字符串比较，提供类型安全性，减少运行时错误风险。
...
CST[CALL_STATUS - 通话状态枚举]
HRT[HANGUP_REASON - 挂断原因枚举]
```

**图表来源**
- [lib/composables/useCallKit.ts:1-254](file://lib/composables/useCallKit.ts#L1-L254)
...
- [lib/types.ts:65-109](file://lib/types.ts#L65-L109)
...
- 处理信令发送
- 触发通话生命周期事件
- **新增功能**：自动解析目标用户的用户资料以确保呼叫等待屏幕显示前的信息可用
...
**章节来源**
- [lib/composables/useCallKit.ts:22-157](file://lib/composables/useCallKit.ts#L22-L157)
- [lib/types.ts:73-109](file://lib/types.ts#L73-L109)

### useEndCall - 通话结束控制

**更新**：useEndCall 增强了枚举状态检查，使用 CALL_STATUS 枚举常量替代字符串比较，提供类型安全性，减少运行时错误风险。
...
**状态检查：**
- `canHangup()` - 检查是否可以挂断，使用 `callStateStore.status !== CALL_STATUS.IDLE && callStateStore.isInCall` 进行类型安全的状态检查
- `canCancel()` - 检查是否可以取消，使用 `callStateStore.status === CALL_STATUS.INVITING` 进行精确的状态匹配

**类型安全改进**：
- 使用 `CALL_STATUS` 枚举常量替代硬编码字符串
- 提供编译时类型检查，避免运行时状态比较错误
- 减少状态字符串拼写错误的风险

**章节来源**
- [lib/composables/useEndCall.ts:1-131](file://lib/composables/useEndCall.ts#L1-L131)
- [lib/types/callstate.types.ts:11-22](file://lib/types/callstate.types.ts#L11-L22)
...
**图表来源**
- [lib/composables/useCallKit.ts:15-254](file://lib/composables/useCallKit.ts#L15-L254)
- [lib/composables/useCallKitEvents.ts:36-142](file://lib/composables/useCallKitEvents.ts#L36-L142)
- [lib/core/events/CallKitEventBus.ts:14-112](file://lib/core/events/CallKitEventBus.ts#L14-L112)
- [lib/services/UserProfileService.ts:49-138](file://lib/services/UserProfileService.ts#L49-L138)
...
Note over CK,UPS : 新增：批量解析被叫方资料
CK->>UPS : resolveUserProfiles([targetId])
UPS->>CSS : setUserInfo() 缓存用户资料
CK-->>VC : Promise
...
6. ****新增功能**：用户资料解析增强，确保呼叫等待屏幕显示前的信息可用
...
**更新**：使用 CALL_STATUS 枚举常量替代字符串比较，提供类型安全性

组件提供了智能的状态检查功能：

- `canHangup()`：使用 `callStateStore.status !== CALL_STATUS.IDLE && callStateStore.isInCall` 进行类型安全的状态检查
- `canCancel()`：使用 `callStateStore.status === CALL_STATUS.INVITING` 进行精确的状态匹配
...
### UserProfileService - 用户资料解析核心

**新增功能**：提供完整的用户资料解析和缓存机制，确保通话发起过程中用户信息的可用性。

**核心功能：**
- 批量解析用户资料（优先缓存 → Provider → 回写缓存）
- 用户资料缓存管理
- 群组资料解析
- Provider 注册和清理

**增强机制**：在 useCallKit 中新增了自动用户资料解析功能，确保呼叫等待屏幕显示前的信息可用。

**章节来源**
- [lib/services/UserProfileService.ts:49-138](file://lib/services/UserProfileService.ts#L49-L138)

### useCallKit 中的用户资料解析流程
...
**图表来源**
- [lib/composables/useCallKit.ts:37-51](file://lib/composables/useCallKit.ts#L37-L51)
...
CK->>CSS : initInviteInfo()
CK->>CK : sendInviteMessage()
Note over CK,UPS : 新增：批量解析被邀请成员资料
CK->>UPS : resolveUserProfiles(members)
UPS->>CSS : setUserInfo() 缓存成员资料
CK->>CSS : setCallStatus(IN_CALL)
CK->>CK : joinChannel()
```

**图表来源**
- [lib/composables/useCallKit.ts:78-90](file://lib/composables/useCallKit.ts#L78-L90)

### 用户资料解析算法

**新增功能**：智能的用户资料解析算法，确保高效和可靠的资料获取。

```mermaid
flowchart TD
Start([开始解析用户资料]) --> CheckCache["检查缓存"]
CheckCache --> CacheHit{"缓存命中?"}
CacheHit --> |是| ReturnCached["返回缓存资料"]
CacheHit --> |否| CheckProvider{"存在Provider?"}
CheckProvider --> |否| ReturnUserId["返回userId兜底"]
CheckProvider --> |是| FetchFromProvider["调用Provider获取资料"]
FetchFromProvider --> WriteCache["写入缓存"]
WriteCache --> ReturnProfiles["返回解析结果"]
ReturnCached --> End([完成])
ReturnUserId --> End
WriteCache --> End
```
...
**新增功能**：通过 GlobalCallStore 提供跨组件的用户资料共享。
...
**新增功能**：EasemobChatCallWaiting 组件现在可以实时显示被叫方的用户资料。
...
**章节来源**
- [lib/components/singleCall/EasemobChatCallWaiting.vue:41-47](file://lib/components/singleCall/EasemobChatCallWaiting.vue#L41-L47)
...
5. ****新增功能**：用户资料解析的错误处理**
   - 用户资料解析失败时会回退到 userId
   - 记录详细的错误日志但不影响通话发起流程

6. **useEndCall 的类型安全改进**
   - 使用 CALL_STATUS 枚举常量替代字符串比较
   - 提供编译时类型检查，减少运行时错误风险
...
6. **useEndCall 的类型安全增强**
   - 使用枚举常量替代字符串比较，提高代码可靠性
   - 减少状态检查错误的风险

**章节来源**
- [lib/composables/useAnswerCall.ts:67-73](file://lib/composables/useAnswerCall.ts#L67-L73)
- [lib/composables/useJoinChannel.ts:185-190](file://lib/composables/useJoinChannel.ts#L185-L190)
- [lib/composables/useCallKit.ts:42-47](file://lib/composables/useCallKit.ts#L42-L47)
- [lib/composables/useCallKit.ts:82-87](file://lib/composables/useCallKit.ts#L82-L87)
- [lib/composables/useEndCall.ts:104-115](file://lib/composables/useEndCall.ts#L104-L115)
...
function resolveUserProfiles
function registerUserInfoProvider
}
CALL_SERVICE {
object instance
}
CALL_STATE ||--|| GLOBAL_CALL : "用户资料缓存"
GLOBAL_CALL ||--|| USER_PROFILE_SERVICE : "资料解析"
...
4. ****新增功能**：用户资料解析的缓存机制**：通过 GlobalCallStore 缓存用户资料，避免重复请求
...
- 及时清理定时器和事件监听器
- 避免循环引用和内存泄漏
- ****新增功能**：用户资料缓存的内存优化**：智能缓存策略，避免无限增长
...
1. **缓存优先策略**：优先使用缓存资料，减少网络请求
2. **批量解析**：支持批量用户资料解析，减少请求次数
3. **Provider 缓存**：Provider 返回的资料会被缓存
4. **错误隔离**：用户资料解析失败不影响通话发起流程

### useEndCall 类型安全优化

**更新**：使用 CALL_STATUS 枚举常量替代字符串比较，提供性能和类型安全双重优势
...
**症状**：呼叫等待屏幕显示用户名字而不是用户ID

**解决方案**：
- 检查 Provider 是否正确注册
- 验证用户资料是否正确返回
- 确认 GlobalCallStore 缓存是否正常工作

**更新**：新增的用户资料解析功能提供了完整的错误处理和回退机制
...
**更新**：用户资料解析提供了智能的缓存策略和更新机制

#### 11. **新增功能**：useEndCall 状态检查错误

**症状**：状态检查逻辑出现类型错误或运行时错误

**解决方案**：
- 确保使用 `CALL_STATUS` 枚举常量进行状态比较
- 验证状态值的有效性
- 检查类型导入是否正确

**更新**：使用枚举常量替代字符串比较，提供类型安全的状态检查

#### 12. **新增功能**：类型安全问题

**症状**：编译时出现类型错误，特别是在状态检查和事件处理中

**解决方案**：
- 确保正确导入 `CALL_STATUS` 和 `HANGUP_REASON` 类型
- 验证枚举常量的使用方式
- 检查类型定义的完整性

**更新**：新增的类型安全改进提供了更好的开发体验和代码质量保证
...
6. ****新增功能**：用户资料解析增强，自动解析目标用户的用户资料以确保呼叫等待屏幕显示前的信息可用**
7. **useEndCall 类型安全增强**：使用 CALL_STATUS 枚举常量替代字符串比较，提供类型安全性，减少运行时错误风险
...
8. ****新增功能**：智能的用户资料解析和缓存机制，提升用户体验**
9. **类型安全增强**：useEndCall 使用枚举常量提供编译时类型检查
...
9. ****新增功能**：合理使用用户资料解析功能，确保用户信息的及时更新**
10. **类型安全编程**：在状态检查中使用枚举常量替代字符串比较
...
6. ****新增功能**：从简单的通话控制到完整的用户资料管理和服务**
7. **从字符串比较到枚举常量**：useEndCall 提供类型安全的状态检查机制

该 API 为构建高质量的实时通信应用提供了坚实的基础，新增的事件系统、结构化参数、错误处理机制、用户资料解析功能和类型安全改进进一步提升了系统的稳定性和用户体验。开发者可以根据具体需求进行定制和扩展，充分利用新增的功能特性。
...
// 呼叫等待屏幕将自动显示用户资料
// 即使 Provider 未返回资料，也会显示 userId 作为回退

// 类型安全的状态检查
const { canHangup, canCancel } = useEndCall()
if (canHangup()) {
  // 使用枚举常量进行状态检查，提供类型安全保障
  await hangupCall()
}
```