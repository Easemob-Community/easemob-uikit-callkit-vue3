...
**更新** 本次更新特别强化了监听器管理的架构设计，通过引入 GlobalCallStore 和 GroupCallStore 实现了更清晰的状态分离。useListenerManager 现在专注于监听器注册和消息处理，而状态管理职责被合理分配到相应的存储模块中。同时，修正了组ID字段路径，提升了多通话场景下的组识别准确性。
...
**更新** 新增了用户属性管理功能，通过 GlobalCallStore 统一管理用户信息。优化了群组通话的参与者生命周期管理，移除了对 callStateStore 中群组属性的直接操作，实现了更清晰的状态分离。修正了组ID字段路径，从 ext.groupId 更正为 ext.callkitGroupInfo.groupId。

**章节来源**
- [lib/composables/useListenerManager.ts:37-262](file://lib/composables/useListenerManager.ts#L37-L262)
...
**图表来源**
- [lib/composables/useListenerManager.ts:179-196](file://lib/composables/useListenerManager.ts#L179-L196)
...
**图表来源**
- [lib/composables/useListenerManager.ts:56-196](file://lib/composables/useListenerManager.ts#L56-L196)
...
**图表来源**
- [lib/composables/useListenerManager.ts:141-194](file://lib/composables/useListenerManager.ts#L141-L194)
...
**图表来源**
- [lib/composables/useListenerManager.ts:141-197](file://lib/composables/useListenerManager.ts#L141-L197)
...
9. **组ID字段路径优化**
   - **更新**：采用 ext.callkitGroupInfo.groupId 确保多通话场景下的正确组识别
   - **更新**：提升组信息获取的准确性和可靠性
   - **更新**：优化组ID解析逻辑，减少字段路径错误的风险
...
**更新** 新增了用户属性处理和 GlobalCallStore 管理的清理步骤，实现了更完整的资源管理。修正了组ID字段路径，提升了多通话场景下的组识别准确性。
...
**更新** 新增了用户信息管理和状态分离相关的故障排除指南，包括 GlobalCallStore 和 GroupCallStore 的诊断和解决方案。修正了组ID字段路径相关的故障排除方法。
...
**更新** 本次更新显著增强了监听器管理的架构设计，通过引入 GlobalCallStore 和 GroupCallStore 实现了更清晰的状态分离。useListenerManager 现在专注于监听器注册和消息处理，而状态管理职责被合理分配到相应的存储模块中。同时，修正了组ID字段路径，提升了多通话场景下的组识别准确性。这种设计不仅提高了代码的可维护性，还为未来的功能扩展奠定了坚实的基础。
