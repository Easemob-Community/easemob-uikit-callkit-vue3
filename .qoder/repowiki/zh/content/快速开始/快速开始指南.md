...
**图表来源**
- [lib/index.ts:1-92](file://lib/index.ts#L1-L92)
...
通过以上步骤，您已经完成了 Easemob Chat CallKit Vue3 的基本配置和使用。以下是关键要点：

1. **安装依赖**：确保 Vue 3、Pinia、环信 IM SDK 和声网 RTC SDK 已正确安装
2. **Provider 配置**：在应用顶层使用 EasemobChatCallKitProvider 包裹应用
3. **组件使用**：在合适的位置放置 InvitationNotification、EasemobChatSingleCall 和 EasemobChatMultiCall 组件
4. **通话控制**：使用 useCallKit hook 进行通话操作
5. **状态管理**：利用 Pinia store 管理通话状态
6. **事件监听**：通过 useCallKitEvents 监听通话生命周期事件
