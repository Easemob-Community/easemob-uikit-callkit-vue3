...
**更新** 现在支持简化的插件注册方式，可以直接使用`app.use(EasemobChatCallKit)`而无需手动配置Pinia，大大简化了安装和配置流程。库内部已集成Pinia管理机制，无需用户手动安装和配置Pinia。
...
**重要说明**：现在无需手动安装 Pinia，库内部已集成 Pinia 管理机制。
...
**重要说明**：插件会自动检测并注入Pinia，如果应用中已有Pinia实例则不会重复创建。库内部已集成 Pinia 管理机制，无需用户手动安装和配置。

**章节来源**
- [QUICK_START.md:21-46](file://QUICK_START.md#L21-L46)
...
**章节来源**
- [QUICK_START.md:49-81](file://QUICK_START.md#L49-L81)
...
**章节来源**
- [QUICK_START.md:84-143](file://QUICK_START.md#L84-L143)
...
3. **内部Pinia管理**：库内部已集成 Pinia 管理机制，无需用户手动处理
4. **保持功能完整**：所有核心功能保持不变，包括通话控制、状态管理和事件监听
...
**章节来源**
- [QUICK_START.md:149-153](file://QUICK_START.md#L149-L153)
